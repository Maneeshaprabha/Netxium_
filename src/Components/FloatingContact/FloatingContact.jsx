...
import { Mail, X } from 'lucide-react'; // Added X icon for a cleaner close button

export default function FloatingContact() {
  const [isModalOpen, setIsModalOpen] = useState(false);

  // Lock body scroll when modal is open (crucial for mobile experience)
  useEffect(() => {
    if (isModalOpen) {
  ...
  return (
    <>
      {/* Fixed positioning wrapper for the button - adjusted bottom spacing for mobile */}
      <div className="fixed bottom-6 md:bottom-10 left-1/2 -translate-x-1/2 z-50 w-[90%] max-w-fit flex justify-center">
        ...
        >
          {/* Text Content - Resizes on mobile to prevent overflow */}
      ...
      <AnimatePresence>
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 font-sans">
            ...
            {/* Modal Content Box - Added max-h and overflow-y-auto for mobile keyboards */}
            <motion.div
            ...
            >
              ...
              <form className="flex flex-col gap-3 sm:gap-4">
                ...
                <button 
                  type="button"
                  className="w-full mt-2 sm:mt-4 bg-white text-black font-medium py-3.5 sm:py-4 rounded-xl hover:bg-gray-200 active:scale-[0.98] transition-all text-sm sm:text-base shadow-lg"
                >
                  Send Message
                </button>
              </form>
